...
df2 = pd.read_csv(
    "data/novas_metricas_clientes_transformadas.csv"
)  # Carrega o DataFrame a partir do arquivo CSV

# Certifique-se de que as datas estão no formato datetime
df2["Data_Vendas"] = pd.to_datetime(
    df2["Data_Vendas"], errors="coerce"
)  # Converte a coluna 'Data_Vendas' para o tipo datetime

# Filtrar apenas as colunas necessárias
df2 = df2[
    ["Nome_Clientes", "Data_Vendas", "Satisfação"]
]  # Seleciona apenas as colunas necessárias

# Agrupar dados por ano e mês
df2["Ano_Mes"] = df2["Data_Vendas"].dt.to_period(
    "M"
)  # Agrupa as datas por ano e mês

# Agrupar dados por Ano_Mes e Satisfação e contar a quantidade de ocorrências
df_grouped = (
    df2.groupby(["Ano_Mes", "Satisfação"]).size().reset_index(name="counts")
)  # Agrupa e conta as ocorrências
...
sns.barplot(
    data=df_grouped, x="Ano_Mes", y="counts", hue="Satisfação"
...
plt.title(
    "Contagem das Datas por Categoria", color="white"
)  # Define o título do gráfico
plt.xticks(
    rotation=40, color="white"
)  # Rotaciona e colore os rótulos do eixo x
plt.yticks(color="white")  # Colore os rótulos do eixo y
plt.legend(
    title="Pesquisa de Satisfação", facecolor="black", edgecolor="black"
)  # Adiciona uma legenda com fundo preto
