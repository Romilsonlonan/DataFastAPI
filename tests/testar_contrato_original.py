...
warnings.simplefilter(
    "always"
)  # Garante que todos os warnings sejam sempre mostrados
...
def test_contrato_original():
    ...
    )  # Loga o tempo de execução inicial

    satisfacoes = [
        "Ruim",
        "Excelente",
        "Bom",
    ]  # Lista de diferentes níveis de satisfação
    ...
    for satisfacao in satisfacoes:
        ...
        df_test = pd.DataFrame(
            {
                "Classificacao_Clientes": [
                    "Clientes_Ouro>=1001.00",
                    "Clientes_Prata<=1000.00",
                    "Clientes_Ouro>=1001.00",
                ],
                "Nome_Clientes": [
                    "Vicente Pinheiro",
        ...
        try:
            ...
            logger.info(
                f"Validação bem-sucedida para a satisfação cliente: {satisfacao}"
            )  # Loga a validação bem-sucedida
        except pa.errors.SchemaError as e:
            logger.error(
                f"Erro na validação para a satisfação cliente: {satisfacao} - {e}"
            )  # Loga o erro de validação
            logger.error(
                e.failure_cases
            )  # Loga os casos de falha de validação
