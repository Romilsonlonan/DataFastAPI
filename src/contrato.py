...
class MetricasClientesBase(pa.DataFrameModel):
    ...
    Classificacao_Clientes: Series[str]  # Coluna para a classificação dos clientes
    ...
    Custo_Unitario: Series[float] = pa.Field(ge=0)  # Coluna para o custo unitário, com valor mínimo de 0
    Preco_Unitario: Series[float] = pa.Field(ge=0)  # Coluna para o preço unitário, com valor mínimo de 0
    Qtd_Vendida: Series[int] = pa.Field(gt=0)  # Coluna para a quantidade vendida, com valor maior que 0
    ...
    @pa.check(
        ...
        error="Checagem de Pesquisa de Satisfação Clientes é Inválido"  # Mensagem de erro se a verificação falhar
    )
...
class NovasMetricasClienteBase(MetricasClientesBase):
    ...
    Custo_Total_Unitario: Series[float] = pa.Field(ge=0, description="Custo total unitário do produto")
    Preco_Total_Unitario: Series[float] = pa.Field(ge=0, description="Preço total unitário do produto")
    Sem_ICMS_Custo: Series[float] = pa.Field(ge=0, description="Custo sem ICMS")
    Sem_ICMS_Preco: Series[float] = pa.Field(ge=0, description="Preço sem ICMS")
    Dif_ICMS_Custo: Series[float] = pa.Field(ge=0, description="Diferença de ICMS no custo")
    Dif_ICMS_Preco: Series[float] = pa.Field(ge=0, description="Diferença de ICMS no preço")
    ...
    def analisar_aumento_preco(df2: pd.DataFrame) -> pd.DataFrame:
        ...
        df2['Percentual_Aumento_Preco'] = (
            (df2['Preco_Unitario'] - df2['Sem_ICMS_Preco']) / df2['Sem_ICMS_Preco'] * 100
        )
        return df2
